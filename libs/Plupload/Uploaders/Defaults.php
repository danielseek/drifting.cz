...
class Defaults extends \Nette\Object implements IUploader
{
    ...
    public function setToken($token = 0)
    {
        $this->token = $token;
        return $this;
    }
    ...
    /*********** Upload handler ***********/
    public function upload($formToken)
    {
        $this->isReady();
        ...
        if (isset($contentType) and strpos($contentType, "multipart") !== false) {
		...
		echo $formToken; flog($formToken);
		$this->onSuccess->invokeArgs(array($upload, $formToken));
